export namespace Y {
    ...
    export function hasClass(element:HTMLElement ,clsname:string):boolean{
        let clsText:string = element.className;
        if(!clsText) return false;
        if(!clsname) return false;
        clsname = clsname.replace(trimRegex,"");
        if(!clsname) return false;
        let at = 0;
        while(true){
            at = clsText.indexOf(clsname,at);
            let lastAt= at + clsname.length;
            ...
            if(lastAt<clsText.length){
                ...
                let ch = clsText[lastAt];
                if(ch!==" " && ch!=="\r" && ch!=="\t" && ch!=="\n"){at = lastAt-1 ;continue;}
            }
            return true;
        }
    }
    export function addClass(element:HTMLElement ,clsname:string){
        if(hasClass(element,clsname)) return false;
        element.className = element.className + " " + clsname;
        return true;
    }
    export function removeClass(element:HTMLElement ,clsname:string){
        let clsText:string = element.className;
        if(!clsText) return false;
        if(!clsname) return false;
        clsname = clsname.replace(trimRegex,"");
    ...
    let valueBinder:IBinder = binders["y-value"] = function(element:HTMLElement,bindable:IBindable,context:View){
        let tag = element.tagName;
        if(tag==="TEXTAREA") return binders["y-value.textbox"](element,bindable,context);
        if(tag==="SELECT") return binders["y-value.select"](element,bindable,context);
        if(tag==="INPUT"){
            let t = (element as HTMLInputElement).type;
            if(t==="radio") return binders["y-value.radio"](element,bindable,context);
            if(t==="checkbox") return binders["y-value.checkbox"](element,bindable,context);
            return txtboxBinder(element,bindable,context);
        }
        textBinder(element,bindable,context);
    }
    valueBinder.parse = function(valueExpr:Expression,opts:ParseViewOpts,binder:IBinder):Expression{
        let element :HTMLElement = opts.element;
        var tag = element.tagName;
        let bindername :string ;
        if(tag==="TEXTAREA") bindername = "y-value.textbox";
        else if(tag==="SELECT") bindername = "y-value.select";
        else if (tag==="INPUT"){
            var t = (element as HTMLInputElement).type;
            if(t==="radio") bindername = "y-value.radio";
            else if(t==="checkbox") bindername = "y-value.checkbox";
            else bindername = "y-value.textbox";
        }
        else bindername = "y-text";
        return new BindExpression(bindername,valueExpr);
    }
    binders["y-value.select"] = function(element:HTMLElement,bindable:IBindable,context:View){
	    let evtHandler = () => { 
                bindable.set_value(
	    ...
	    let setValue:Function =  (element:HTMLElement, value:any)=> {
    ...
    let txtboxBinder = binders["y-value.textbox"] =function(element:HTMLElement,bindable:IBindable,context:View){
        let onchange = function () { tick = 0; bindable.set_value((element as HTMLInputElement).value); }
	    let tick:number;
	    ...
	    bindable.subscribe(handler);
	    (element as HTMLInputElement).value = bindable.get_value();
    }
    binders["y-value.radio"] = function(element:HTMLElement,bindable:IBindable,context:View){
        let evtHandler = function () {
	        if ((element as HTMLInputElement).checked) bindable.set_value((element as HTMLInputElement).value);
	        else bindable.set_value(null);
	    }
	    ...
	    bindable.subscribe(handler);
	    setValue((element as HTMLInputElement), bindable.get_value());
    };
    binders["y-value.checkbox"] =function(element:HTMLElement,bindable:IBindable,context:View){
        let evtHandler = function () {
    ...
    binders["y-visible"] = function (element:HTMLElement,bindable:IBindable,context:View) {
        let setValue=(element:HTMLElement ,value:any)=>{
	    ...
	    bindable.subscribe((sender,evt)=>{
            setValue(element, evt.value);
        });
        if(element.getAttribute("y-visible-nobound")!==null) setValue(element, bindable.get_value());
    }

    binders["y-disable"] = function (element:HTMLElement,bindable:IBindable,context:View) {
        let setValue=(element:HTMLElement ,value:any)=>{
    ...
    binders["y-event"] = function (element:HTMLElement,bindable:IBindable,context:View) {
        let value = bindable.get_value();
        let evtname :string;let evthandler : string;
        if(typeof value==="string"){
            evtname = "click";evthandler = value;
        }else{
            evtname = value.name;
            evthandler = value.handler;
            if(!evtname) evtname = "click";
        }
        let controller :any= context.controller;
        let handler = controller[evthandler];
        if(typeof handler==="function"){
            platform.attach(element , evtname,handler as Function);
        }
    }
